class Car {
   ...
   update(
      ...
      if (!this.damaged) {
         ...
         this.damaged = this.#assessDamage(roadBorders, carBorders);
         if (this.damaged) {
            this.speed = 0;
      ...
      if (this.sensor) {
         ...
         if (this.brainOptions.extraInputs.includes("🚙")) {
            offsets.push(Math.max(...carOffsets));
         }
         ...
         this.inputs = offsets;
         /*const old_outputs = NeuralNetwork.feedForward(
                    offsets,
                    this.brain
                );*/
   ...
   draw(ctx, optimize = false) {
      ...
      if (this.stopSensor && !optimize) {
         this.stopSensor.draw(ctx, "red", false);
     }
     if (this.lightSensor && !optimize) {
         this.lightSensor.draw(ctx,  "red", false);
     }
