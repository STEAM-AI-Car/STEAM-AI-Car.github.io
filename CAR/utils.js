...
function generateImages(emojis, size = 30) {
   const result = {};
   for (const color of ["yellow", "cyan","gray","red","green","blue"]) {
...
function convert64(emojis, base64Result){
   const result = {};
   for (const color of ["yellow", "cyan","gray","red","green","blue"]) {
      for (const emoji of emojis) {
         ...
         const img = new Image();
         img.src = "data:image/png;base64," + base64Result[emoji][color];
         result[emoji][color] = img;
      }
   }
   return result;
...
function generateImages64(emojis, size = 30) {
   const result = {};
   for (const color of ["yellow", "cyan","gray","red","green","blue"]) {
      for (const emoji of emojis) {
         const canvas = document.createElement("canvas");
         ...
         */
         const dataUrl = canvas.toDataURL();
         result[emoji][color] = dataUrl.split(',')[1];
      }
   }
   return result;
}
