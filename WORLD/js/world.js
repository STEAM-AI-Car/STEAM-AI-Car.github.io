class World {
    ...
    static load(info) {
      ...
      */
        ...
        for(let i=0;i<world.roadBorders.length-1;i++){
         for(let j=i+1;j<world.roadBorders.length;j++){
            const seg1=world.roadBorders[i];
            const seg2=world.roadBorders[j];
            if(seg1.p1.almostEquals(seg2.p1)){
               seg1.p1=seg2.p1;
            }
            if(seg1.p1.almostEquals(seg2.p2)){
               seg1.p1=seg2.p2;
            }
            if(seg1.p2.almostEquals(seg2.p1)){
               seg1.p2=seg2.p1;
            }
            if(seg1.p2.almostEquals(seg2.p2)){
               seg1.p2=seg2.p2;
            }
         }
        }
    ...
    getNearbyItemBorders(car) {
        ...
        return (data.buildings.concat(data.trees)).map(i=>i.base.segments).flat();
    }
    ...
    generateShortestPathBorders(car, endPoint) {
        ...
        for(let i=0;i<segments.length-1;i++){
         for(let j=i+1;j<segments.length;j++){
            ...
            if(seg1.p1.almostEquals(seg2.p1,1)){
               seg1.p1=seg2.p1;
            }
            if(seg1.p1.almostEquals(seg2.p2,1)){
               seg1.p1=seg2.p2;
            }
            if(seg1.p2.almostEquals(seg2.p1,1)){
               seg1.p2=seg2.p1;
            }
            if(seg1.p2.almostEquals(seg2.p2,1)){
               seg1.p2=seg2.p2;
            }
         }
        }*/
        ...
        return {segs:segments.map((s) => [s.p1, s.p2]),envs:tmpEnvelopes};
    }
    ...
    #drawCars(ctx, optimizing, layerCheck = null) {
        if (optimizing) {
            ...
            ctx.globalAlpha = 1;
            for (const car of this.cars) {
                if (car.marked && car.layer == layerCheck) {
                    car.draw(ctx, true);
                }
            }
            /*if (this.bestCar) {
    ...
    draw(
        ...
        if (!this.grid || useGrid == false) {
            ...
            if (this.drawContents.includes("markings")) {
                for (const marking of this.markings) {
                    if (
                        !(
                            marking instanceof Start //||
                            //marking instanceof Target
                        ) ||
                        showStartMarkings
                    ) {
                     if(marking instanceof Target && defaultOptions.applyShortestPath){
         ...
         for (const env of this.envelopes.filter((e) => e.layer == 1)) {
            ...
            if (this.drawContents.includes("envelopes")) {
                for (const seg of this.graph.segments.filter(
                    (e) => e.layer == 1
                )) {
                    seg.draw(ctx, {
                        color: "#BBB",
                ...
                for (const seg of this.roadBorders.filter(
                    (b) => b.layer == 1
                )) {
                    seg.draw(ctx, { color: "white", width: 4 });
                }
            ...
            if(this.bestCar.envelopes){
               for(const env of this.bestCar.envelopes){
                  env.draw(ctx,{fill:"rgb(0,100,150)",stroke:"rgba(0,0,0,0)"});
               }
            }
            ...
            if (this.drawContents.includes("envelopes")) {
                for (const seg of data.graphSegments) {
                    seg.draw(ctx, { color: "#BBB", width: 4, dash: [10, 10] });
                }
                for (const seg of data.roadBorders) {
                    seg.draw(ctx, { color: "#BBB", width: 4 });
                }
            }
         ...
         */
         ...
         for (const env of data.envelopes.filter(e=>e.layer==1)) {
            ...
            if (this.drawContents.includes("envelopes")) {
                for (const seg of data.graphSegments.filter(
                    (e) => e.layer == 1
                )) {
                    seg.draw(ctx, {
                        color: "#BBB",
                ...
                for (const seg of data.roadBorders.filter(
                    (e) => e.layer == 1
                )) {
                    seg.draw(ctx, { color: "white", width: 4 });
                }
